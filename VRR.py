...
is_process_coming_from_waiting=0
waiting_time=0
turnaround_time=0
def print_waiting_and_turnaround_time(times,turnaround_time,waiting_time,is_process_coming_from_waiting):
    i=0
    ...
    while(remaining_processes!=0 or (i==0 or i==1)):
        if (remaining_time_list[i]<=time_quantm and remaining_time_list[i]>0):
            ...
            if (remaining_time_list[i]>0 and is_process_coming_from_waiting==1):
                remaining_time_list[i]-=time_quantm
                times+=time_quantm
                is_process_coming_from_waiting=0

        if (is_burst_time_smaller_than_time_quantm==1 and remaining_time_list[i]==0 or is_process_coming_from_waiting==1):
...
print_waiting_and_turnaround_time(times,turnaround_time,waiting_time,is_process_coming_from_waiting)
